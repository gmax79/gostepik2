...
* (опционально) Статика встроена прямо в бинарь ( те вообще нет зависимостей ) через https://github.com/shurcooL/vfsgen
...
Это примерно то, чем людит пользуются при отправки решения домашек на разных образовательных платформах. В том числе и вы при обучении на курсе.
...
* Пользователь грузит файл, он добавляется в базу с пометкой "обрабатыватся" и в очередь на обработку.
* Очередь (в виде отдельного сервиса) достает задачу на проверку и отправляет ее во внешний веб-сервис по описанному протоколу
...
* Авторизация - ввод логина и пароля, получение сессии
* Регистрация - ввод логина, пароля, создание пользователя в базе
* Список заданий, куда были загрузки - просто таблица со списком
...
Очередь:
* Простой разгребатор очереди, который берет задание и отправляет во внешнюю систему. Можно взять RabbitMQ, Kafka, Redis Push/Sum, очередь на MySQL или PostgreSQL.

Внешний грейдер:
* API, который принимает решение на проверку от основного сервера. Реализовано в виде HTTP-сервера
...
* Вход через oauth ( мейл, ВК, фейсбук) - https://github.com/golang/oauth2
* Авторизация-регистрация в виде отдельного микросервиса, реализованного через gRPC или swagger (с подходом schems-first, когда вы из спецификации генерируете и сервер, и клиент)
* Несколько языков программирования в грейлере